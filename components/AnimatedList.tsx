...
function AnimatedList<T>({
  ...
  // Default render for string items (backwards compatible)
  const defaultRenderItem = (item: T, index: number, isSelected: boolean): ReactNode => {
    return (
    ...
    <div className={`scroll-list-container ${className}`}>
      <div ref={listRef} className={`scroll-list ${!displayScrollbar ? 'no-scrollbar' : ''}`} onScroll={handleScroll}>
        {items.map((item, index) => (
          ...
          >
            {itemRenderer(item, index, selectedIndex === index)}
          </AnimatedItem>
        ))}
      </div>
      {showGradients && (
